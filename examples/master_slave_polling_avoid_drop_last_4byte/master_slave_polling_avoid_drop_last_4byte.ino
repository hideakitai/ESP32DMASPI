...
static const uint32_t TARGET_SIZE = 8192;
static const uint32_t BUFFER_SIZE = TARGET_SIZE + 4;  // send and receive 4 bytes more
...
void loop() {
    ...
    while (slave.available()) {
        ...
        if (memcmp(spi_slave_rx_buf, spi_master_tx_buf, TARGET_SIZE)) {
            printf("[ERROR] Master -> Slave Received Data has not matched !!\n");
            cmp_bug("Received ", spi_slave_rx_buf, "Sent ", spi_master_tx_buf, TARGET_SIZE);
        }

        if (memcmp(spi_master_rx_buf, spi_slave_tx_buf, TARGET_SIZE)) {
            printf("ERROR: Slave -> Master Received Data has not matched !!\n");
            cmp_bug("Received ", spi_master_rx_buf, "Sent ", spi_slave_tx_buf, TARGET_SIZE);
        }
